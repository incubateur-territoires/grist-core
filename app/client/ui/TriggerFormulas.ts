...
const translate = (x: string, args?: any): string => t(`TriggerFormulas.${x}`, args);
...
function buildTriggerSelectors(ctl: IOpenController, tableRec: TableRec, column: ColumnRec,
  ...
  return cssSelectorMenu(
    ...
    cssItemsFixed(
      cssSelectorItem(
        labeledSquareCheckbox(current,
          [translate('CurrentField'), cssSelectorNote('(data cleaning)')],
      ...
      cssSelectorItem(
        labeledSquareCheckbox(allUpdates,
          [translate('AnyField'), cssSelectorNote('(except formulas)')]
        ),
      ),
    ),
    cssItemsList(
    ...
    cssItemsFixed(
      cssSelectorFooter(
        dom.maybe(isChanged, () =>
          primaryButton(translate('OK'),
            dom.on('click', () => close(true)),
            testId('trigger-deps-apply')
          ),
        ),
        basicButton(dom.text(use => use(isChanged) ? translate('Cancel') : translate('Close')),
