...
const translate = (x: string, args?: any): string => t(`ViewSectionMenu.${x}`, args);

const TOOLTIP_DELAY_OPEN = 750;

// Handler for [Save] button.
async function doSave(docModel: DocModel, viewSection: ViewSectionRec): Promise<void> {
  await docModel.docData.bundleActions(translate("UpdateSortFilterSettings"), () => Promise.all([
        ...
        menu(ctl => [
          ...
          dom.domComputed(displaySaveObs, displaySave => [
            displaySave ? cssMenuInfoHeader(
              cssSaveButton(translate('Save'), testId('btn-save'),
                            dom.on('click', () => { save(); ctl.close(); }),
                            dom.boolAttr('disabled', isReadonly)),
              basicButton(translate('Revert'), testId('btn-revert'),
...
function makeSortPanel(section: ViewSectionRec, sortSpec: Sort.SortSpec, getColumn: (row: number) => ColumnRec) {
  ...
  return [
    cssMenuInfoHeader(translate('SortedBy'), testId('heading-sorted')),
...
export function makeAddFilterButton(viewSectionRec: ViewSectionRec, popupControls: WeakMap<ColumnRec, PopupControl>) {
  return dom.domComputed((use) => {
    ...
    return cssMenuText(
      ...
      cssMenuTextLabel(translate('AddFilter')),
    );
  });
}
...
export function makeFilterBarToggle(activeFilterBar: CustomComputed<boolean>) {
  return cssMenuText(
    ...
    cssMenuTextLabel(translate("ToggleFilterBar")),
  );
}
...
function makeFilterPanel(section: ViewSectionRec, activeFilters: FilterInfo[],
  ...
  return [
    cssMenuInfoHeader(translate('FilteredBy'), {style: 'margin-top: 4px'}, testId('heading-filtered')),
...
function makeCustomOptions(section: ViewSectionRec) {
  ...
  const text = Computed.create(null, use => {
    if (use(section.activeCustomOptions)) {
      return use(section.activeCustomOptions.isSaved) ? translate("Customized") : translate("Modified");
    } else {
      return translate("Empty");
    }
  });
  return [
    cssMenuInfoHeader(translate('CustomOptions'), testId('heading-widget-options')),
    cssMenuText(
