...
const translate = (x: string, args?: any): string => t(`ui2018.breadcrumbs.${x}`, args);
...
const fiddleExplanation = translate('FiddleExplanation');
    ...
    return cssBreadcrumbs(
      ...
      dom.domComputed((use) => {
        if (options.isSnapshot && use(options.isSnapshot)) {
          return cssTag(translate('Snapshot'), testId('snapshot-tag'));
        }
        if (use(options.isFork)) {
          return cssTag(translate('Unsaved'), testId('unsaved-tag'));
        }
        if (use(options.isRecoveryMode)) {
          return cssAlertTag(translate('RecoveryMode'),
        ...
        if (userOverride) {
          return cssAlertTag(userOverride.user?.email || translate('Override'),
            dom('a',
        ...
        if (use(options.isFiddle)) {
          return cssTag(translate('Fiddle'), tooltip({title: fiddleExplanation}), testId('fiddle-tag'));
        }
      }),
      separator(' / ',
                testId('bc-separator'),
