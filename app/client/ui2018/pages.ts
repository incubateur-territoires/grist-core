...
const translate = (x: string, args?: any): string => t(`ui2018.pages.${x}`, args);
...
export function buildPageDom(name: Observable<string>, actions: PageActions, ...args: DomElementArg[]) {
  ...
  const pageMenu = () => [
    menuItem(() => isRenaming.set(true), translate("Rename"), testId('rename'),
            dom.cls('disabled', actions.isReadonly)),
    menuItem(actions.onRemove, translate('Remove'), testId('remove'),
             dom.cls('disabled', (use) => use(actions.isReadonly) || actions.isRemoveDisabled())),
    menuItem(actions.onDuplicate, translate('DuplicatePage'), testId('duplicate'),
             dom.cls('disabled', actions.isReadonly)),
    dom.maybe(actions.isReadonly, () => menuText(translate('NoEditAccess'))),
  ];
