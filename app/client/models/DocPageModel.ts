...
const translate = (x: string, args?: any): string => t(`models.DocPageModel.${x}`, args);
...
export class DocPageModelImpl extends Disposable implements DocPageModel {
  ...
  public offerRecovery(err: Error) {
    ...
    confirmModal(
      translate("ErrorAccessingDocument"),
      translate("Reload"),
      async () => window.location.reload(true),
      isDocOwner ? translate('ReloadingOrRecoveryMode', {error: err.message}) :
        translate('AccessError', {context: isDenied ? 'denied' : 'recover', error: err.message}),
      {  hideCancel: true,
         extraButtons: (isDocOwner && !isDenied) ? bigBasicButton(translate('EnterRecoveryMode'), dom.on('click', async () => {
...
function addMenu(importSources: ImportSource[], gristDoc: GristDoc, isReadonly: boolean): DomElementArg[] {
  ...
  return [
    menuItem(
      ...
      menuIcon("Page"), translate("AddPage"), testId('dp-add-new-page'),
    ...
    menuItem(
      ...
      menuIcon("Widget"), translate("AddWidgetToPage"), testId('dp-add-widget-to-page'),
    ...
    menuItem(() => gristDoc.addEmptyTable().catch(reportError),
      menuIcon("TypeTable"), translate("AddEmptyTable"), testId('dp-empty-table'),
      dom.cls('disabled', isReadonly)
    ),
    menuDivider(),
    ...
    isReadonly ? menuText(translate('NoEditAccess')) : null,
