...
const translate = (x: string, args?: any): string => t(`ChartView.${x}`, args);
...
export class ChartConfig extends GrainJSDisposable {
    ...
    return [
      ...
      dom.maybe((use) => ['line', 'bar'].includes(use(this._section.chartTypeDef)), () => [
        ...
        dom.domComputed(this._optionsObj.prop('errorBars'), (value: ChartOptions["errorBars"]) =>
          value === 'symmetric' ? cssRowHelp(translate('EachYFollowedByOne')) :
          value === 'separate' ? cssRowHelp(translate('EachYFollowedByTwo')) :
          null
        ),
      ]),
      ...
      dom.maybe(this._groupData, () => [
        ...
        cssHintRow(translate('CreateSeparateSeries')),
      ]),
      ...
      cssRow(
        select(
          ...
          { defaultLabel: translate('PickColumn') }
        ),
        testId('x-axis'),
      ),
  ...
  private async _setGroupDataColumn(colId: string) {
    ...
    await this._gristDoc.docData.bundleActions(translate('SelectedNewGroupDataColumnd'), async () => {
      this._freezeXAxis.set(true);
      this._freezeYAxis.set(true);
      try {
  ...
  private async _setAggregation(val: boolean) {
    try {
      this._freezeXAxis.set(true);
      await this._gristDoc.docData.bundleActions(translate("ToggleChartAggregation"), async () => {
        if (val) {
