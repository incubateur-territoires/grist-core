...
const translate = (x: string, args?: any): string => t(`ActionLog.${x}`, args);
...
export class ActionLog extends dispose.Disposable implements IDomComponent {
  ...
  private _buildLogDom() {
    this._loadActionSummaries().catch((error) => gristNotify(translate("ActionLogFailed")));
    ...
    for (let i = index; i >= 0; i--) {
      ...
      if (tableRename) {
        ...
        if (!newName) {
          ...
          gristNotify(translate('TableRemovedInAction', {tableId:tableId, actionNum: action.actionNum}));
          return;
        }
        tableId = newName;
      }
      ...
      if (td.removeRows.indexOf(rowId) >= 0) {
          // TODO - find a better way to send informative notifications.
        gristNotify(translate("RowRemovedInAction", {actionNum}));
        return;
      }
      ...
      if (columnRename) {
        ...
        if (!newName) {
          // TODO - find a better way to send informative notifications.
          gristNotify(translate("ColumnRemovedInAction", {colId, actionNum: action.actionNum}));
          return;
        }
        colId = newName;
      }
