...
const translate = (x: string, args?: any): string => t(`Drafts.${x}`, args);
...
class NotificationAdapter extends Disposable implements Notification {
  ...
  public showUndoDiscard() {
    ...
    const notification = notifier.createUserMessage(translate("UndoDiscard"), {
      message: () =>
        discardNotification(
          dom.on("click", () => {
            this._hadAction = true;
...
function cellTooltip(clb: () => any) {
  return function (ctl: ITooltipControl) {
    return styledTooltip(
      cssLink(translate('RestoreLastEdit'),
